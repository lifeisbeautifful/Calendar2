...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        {
            string connection = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\Oksana\source\repos\WindowsFormsApp1\WindowsFormsApp1\Database1.mdf;Integrated Security=True";
            ...
            SqlSelect(new DateTime(Year,Month,1).AddDays(-1).ToString(),start.ToString());
        }
        ...
        private void NavigateToChosenDate(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlSelect(new DateTime(userDate.Year,userDate.Month,1).AddDays(-1).ToString(),new DateTime(userDate.Year,userDate.Month,1).AddMonths(1).ToString());
        ...
        private void buttonSaveClick(object sender, EventArgs e)
        {
            ...
            SqlCommand command2 = new SqlCommand("SELECT Dates, Note FROM Notes order by Dates", sqlConnection);
        ...
        {
            SqlDataReader sqlReader = null;
            SqlCommand command = new SqlCommand("SELECT Dates, Note FROM Notes WHERE Dates between @Start and @End order by Dates", sqlConnection);
        ...
        private void SqlUpdate()
        {
            for (int i = 1; i < 32; i++)
            {
                if ((Controls["richTextBox" + i] as RichTextBox).Text != "")
                {
                    SqlCommand command = new SqlCommand("UPDATE [Notes] SET [Note]=@Note WHERE [Dates]=@Dates ", sqlConnection);
        ...
        {
            SqlCommand command3 = new SqlCommand("INSERT INTO [Notes] (Dates,Note)VALUES(@Dates,@Note)", sqlConnection);
        ...
        {
            SqlCommand command4 = new SqlCommand("DELETE FROM [Notes] WHERE [Dates]=@Dates", sqlConnection);
